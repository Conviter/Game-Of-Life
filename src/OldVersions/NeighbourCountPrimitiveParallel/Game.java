...
public class Game {
    ...
    public class ApplyRulesTask extends RecursiveTask<Long2IntOpenHashMap> {
        int fromIndex;
        int toIndex;
        Long2IntOpenHashMap localMap = new Long2IntOpenHashMap(startingCellSize);

        public ApplyRulesTask(int from, int to){
            fromIndex = from;
            toIndex = to;
        }

        @Override
        protected Long2IntOpenHashMap compute() {
            if (toIndex - fromIndex <= 100000) {
                for (int i = fromIndex; i < toIndex; i++) {
                    long cell = aliveCells[i];
                    localMap.addTo(cell, 10);
                    ...
                    for (int j = 0; j < neighbourOffsetX.length; j++) {
                        ...
                        localMap.addTo(key, 1);
                    }
                }
                return localMap;
            } else {
                int middle = (fromIndex + toIndex) / 2;
                ApplyRulesTask left = new ApplyRulesTask(fromIndex, middle);
                ApplyRulesTask right = new ApplyRulesTask(middle, toIndex);

                left.fork();
                Long2IntOpenHashMap rightMap = right.compute();
                Long2IntOpenHashMap leftMap = left.join();


                leftMap.forEach(rightMap::addTo);
                return rightMap;
            }
        }
    }
    ...
    public void applyRules() {
        ...
        ApplyRulesTask task = new ApplyRulesTask(0, filledIndex);
        neighbourCount = pool.invoke(task);
